...
AddEventHandler('housing:getGarage', function(house_id, house_model)
    local src = source
    local player = exports['np-base']:GetCurrentCharacterInfo(src)
    exports.ghmattimysql:execute('SELECT * FROM __housekeys WHERE `house_id`= ? AND `house_model`= ?  AND `cid` = ?', {house_id, house_model, player.id}, function(returnData)
    ...
    exports.ghmattimysql:execute('SELECT * FROM __housedata WHERE `house_id`= ? AND `house_model`= ?  AND `cid` = ?', {house_id, house_model, player.id}, function(penis)
        if penis[1] ~= nil then
...
AddEventHandler('house:enterhouse', function(cid,isRealEstate,house_id,house_model,furniture)
    ...
    exports.ghmattimysql:execute('SELECT * FROM __housedata WHERE `house_id`= ? AND `house_model`= ?', {house_id, house_model}, function(data3)
...
AddEventHandler('house:givekey', function(house_id,house_model,house_name,target)
    local src = source
    local firstname, lastname, cid = exports['np-base']:getModule("Player"):getCharacterNameFromSource(tonumber(target))
    exports.ghmattimysql:execute('SELECT `house_id` FROM __housekeys WHERE `cid`= ?', {cid}, function(data)
        ...
        if penis == "[]" then
            exports.ghmattimysql:execute('SELECT `garages` FROM __housedata WHERE `house_id`= ?', {house_id}, function(test)
                ...
                exports.ghmattimysql:execute('INSERT INTO __housekeys(cid, house_id, house_model, housename, name, garages) VALUES(?, ?, ?, ?, ?, ?)', {cid, house_id, house_model, house_name, name, garages})
...
AddEventHandler('house:removeKey', function(house_id, house_model, target)
    local src = source
    local player = exports['np-base']:getModule("Player"):getCharacterFromCid(target)
            ...
            if result[1] ~= nil then
                ...
                if result[1].can_pay ~= "false" then
                    if result[1].due ~= tonumber(0) then
                        if player.cash >= amountdue then
                            TriggerEvent("cash:remove", src, amountdue)
