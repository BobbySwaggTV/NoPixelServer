            ...
            if DoesEntityExist(currVeh) then
                if IsThisModelABoat(vehModel) or IsThisModelAJetski(vehModel) or IsThisModelAnAmphibiousCar(vehModel) or IsThisModelAnAmphibiousQuadbike(vehModel) then
                  ...
                  if IsBoatAnchoredAndFrozen(currVeh) then
                      ...
                      exports['np-sync']:SyncedExecution('SetBoatAnchor', currVeh, false)
                      exports['np-sync']:SyncedExecution('SetBoatFrozenWhenAnchored', currVeh, false)
                      exports['np-sync']:SyncedExecution('SetForcedBoatLocationWhenAnchored', currVeh, false)
                  elseif not IsBoatAnchoredAndFrozen(currVeh) and GetEntitySpeed(currVeh) < 20 then
                      TriggerEvent("DoLongHudText", "Anchor Dropped", 1)
                      exports['np-sync']:SyncedExecution('SetBoatAnchor', currVeh, true)
                      exports['np-sync']:SyncedExecution('SetBoatFrozenWhenAnchored', currVeh, true)
                      exports['np-sync']:SyncedExecution('SetForcedBoatLocationWhenAnchored', currVeh, true)
                  else
                      TriggerEvent("DoLongHudText", "Anchor Failed", 2)
                  end
                end
            end
        end
...
AddEventHandler('food:Fries', function()
  currentValues["hunger"] = currentValues["hunger"] + 20
...
RegisterNetEvent("carhud:enterVehicleSetTurbulence")
AddEventHandler("carhud:enterVehicleSetTurbulence", function(shouldInact)
